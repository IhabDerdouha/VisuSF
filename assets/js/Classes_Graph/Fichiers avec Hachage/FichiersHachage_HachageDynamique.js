...
class Grph_Fichier_TnOF_HachDyn extends Grph_Fichier_TnOF {
    ...
    eclater(nomFichier, nomBloc0, nomBloc1, nomBloc2) {
            ...
            for (j = 0; j < buf0.nb; j++) {
                ...
                if (this.fichier_TnOF.hash(i + 1, e) == n) {
                    ...
                    for (let z = 0; z < tab.length; z++) {
                        ...
                        etape.algorithme = tab[z].algorithme.length > 0 ? "h<sub>i+1</sub>(" + e + ") = " + (n + 1) + " --> " + tab[z].algorithme + " dans buf1" : "";
                        etapes.push(etape);
                    }
                    buf1.inserer(e, j1);
                } else {
                    ...
                    for (let z = 0; z < tab.length; z++) {
                        ...
                        etape.algorithme = tab[z].algorithme.length > 0 ? "h<sub>i+1</sub>(" + e + ") = " + (n1 + 1) + " --> " + tab[z].algorithme + " dans buf2" : "";
                        etapes.push(etape);
                    }
                    buf2.inserer(e, j2);
                }
        ...
        etape = new Etape();
        etape.algorithme = "Pointeur d'Eclat. ++ >> p = " + (this.fichier_TnOF.entete.p+1);
        etapes.push(etape);

        etape = new Etape();
        etape.algorithme = "Test: Pointeur == 2<sup>i</sup> ?";
        etapes.push(etape);

        etape = new Etape();
        if (this.fichier_TnOF.entete.p + 2 == Math.pow(2, this.fichier_TnOF.entete.i)) {
            etape.algorithme += "Pointeur == 2<sup>i</sup> >> p = 0 et Niveau++";
        } else etape.algorithme = "Pointeur != 2<sup>i</sup>";
        etapes.push(etape);
